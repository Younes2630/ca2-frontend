...
document.querySelector("#createBtn").addEventListener('click', function() {
    ...
    fetch("http://localhost:8080/CA2/api/person/", options)
        .then(res => handleHttpErrors(res))
        .then(data => {
            document.getElementById("createRes").innerHTML = "<h1>Registration complete.</h1>";
            console.log(data);
        }).catch(err => {
            if (err.status) {
                if(err.status === 409){
                    document.getElementById("createRes").innerHTML = "<h1>Person already exists</h1>";
                }else if(console.log(400)){
                    document.getElementById("createRes").innerHTML = "<h1>Enter a value at every field</h1>";
                }else{
                    document.getElementById("createRes").innerHTML = "<h1>An error happened, try again</h1>";
                }
            } else {
...
document.querySelector(".find .button").addEventListener('click', function() {
    ...
    fetch(`http://localhost:8080/CA2/api/person/${resource}/${input}`)
        .then(res => handleHttpErrors(res))
        .then(data => {
            res_size.style.margin = "20px";
            res_size.classList.add("m-3");
            let tablehead = "<thead><tr><td>Name</td><td>Email</td><td>Street</td><td>Zipcode</td><td>By</td></tr></thead>";
            let tabledata;
            if (data.length > 1) {
                res_size.innerHTML = `${data.length} results`;
            ...
            <td>${data.email}</td><td>${data.street}</td><td>${data.zip}</td><td>${data.city}</td></tr>` + "</tbody>";
                res_size.innerHTML = `1 result`;
            }
            table.innerHTML = tablehead + tabledata;
        }).catch(err => {
            if (err.status) {
                err.fullError.then(e => table.innerHTML = "<h1> No persons found !</h1>")
            } else {
                console.log("Network error");
            }
        });
